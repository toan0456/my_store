...
const storeRefreshToken = async (userId, refreshToken) => {
    await redis.set(`refreshToken:${userId}`, refreshToken, "EX", 7*24*60*60*1000)// 7 ngay
}
...
const setCookies = (res, accessToken, refreshToken)=> {
    ...
    res.cookie("refreshToken", refreshToken, {
        ...
        secure:process.env.NODE_ENV === "production",
        sameSite: "strict", // prevent CSRF attack, cross site request forgery attack
        maxAge: 7*24*60*60*1000 // 15 phut
    })
}
...
export const signin = async (req, res) => {
    try {
        ...
        res.status(201).json({message: "Dang ky thanh cong", data: {id:_id.user, name: user.name, email: user.email}})
    } catch (error) {
        console.log(error)
